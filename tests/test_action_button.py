...
class TestActionButton(unittest.TestCase):
    ...
    def test_initialization(self):
        """ Makes sure everything is initialized properly. """
    ...
    def test_hover_effect(self):
        """ Makes sure entering and leaving properly turns on and off the hover color. """
    ...
    def test_custom_background_color(self):
        """ Makes sure action buttons can be created with custom background colors. """
        button = ActionButton(self.root, 'Test', bg='#123456')
        button.on_leave(None)
        self.assertEqual(button.itemcget(button.rect, 'fill'), '#123456')

    def test_command_execution(self):
        """ Tests that on_press will execute the given command. """
        mock_command = Mock()
    ...
    def test_icon_creation(self):
        """ Tests that custom icons can be created. """
